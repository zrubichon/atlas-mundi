...
async function dbGet(cacheKey) {
  ...
  try {
    ...
    if (html) console.log('[Supabase] Cache HIT:', cacheKey);
    return html;
  } catch (e) { 
...
module.exports = async function handler(req, res) {
  ...
  // Model selection by content depth needed
  const quickTabs = ['ticker','news','country_news','chat'];
  const deepTabs = ['history','wars','crises','antiquity','leaders','geopolitics'];
  let modelName, maxTok;
  if (quickTabs.includes(tab)) {
    modelName = 'claude-haiku-4-5-20251001'; // Fast: news, chat, ticker
    maxTok = 2000;
  } else if (deepTabs.includes(tab)) {
    modelName = 'claude-sonnet-4-5'; // Deep: history, wars, crises
    maxTok = 4000;
  } else {
    modelName = 'claude-sonnet-4-5'; // Standard: overview, culture, etc
    maxTok = 3000;
  }
